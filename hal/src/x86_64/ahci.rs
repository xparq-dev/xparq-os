...
const AHCI_REG_CAP: usize = 0x00; // Host Capabilities
const AHCI_REG_GHC: usize = 0x04; // Global Host Control
const AHCI_REG_IS: usize = 0x08;  // Interrupt Status
const AHCI_REG_PI: usize = 0x0C;  // Ports Implemented
const AHCI_REG_VS: usize = 0x10;  // Version
const AHCI_REG_CCC_CTL: usize = 0x14; // Command Completion Coalescing Control
const AHCI_REG_CCC_PORTS: usize = 0x18; // Command Completion Coalescing Ports
const AHCI_REG_EM_LOC: usize = 0x1C; // Enclosure Management Location
const AHCI_REG_EM_CTL: usize = 0x20; // Enclosure Management Control
const AHCI_REG_CAP2: usize = 0x24; // Host Capabilities Extended
const AHCI_REG_BOHC: usize = 0x28; // BIOS/OS Handoff Control and Status

// AHCI Port Register Offsets
const AHCI_PORT_REG_CLB: usize = 0x00; // Command List Base Address
const AHCI_PORT_REG_CLBU: usize = 0x04; // Command List Base Address Upper 32 Bits
const AHCI_PORT_REG_FB: usize = 0x08; // FIS Base Address
const AHCI_PORT_REG_FBU: usize = 0x0C; // FIS Base Address Upper 32 Bits
const AHCI_PORT_REG_IS: usize = 0x10; // Interrupt Status
const AHCI_PORT_REG_IE: usize = 0x14; // Interrupt Enable
const AHCI_PORT_REG_CMD: usize = 0x18; // Command and Status
const AHCI_PORT_REG_TFD: usize = 0x20; // Task File Data
const AHCI_PORT_REG_SIG: usize = 0x24; // Signature
const AHCI_PORT_REG_SSTS: usize = 0x28; // SATA Status (SStatus)
const AHCI_PORT_REG_SCTL: usize = 0x2C; // SATA Control (SControl)
const AHCI_PORT_REG_SERR: usize = 0x30; // SATA Error (SError)
const AHCI_PORT_REG_SACT: usize = 0x34; // SATA Active (SActive)
const AHCI_PORT_REG_CI: usize = 0x38; // Command Issue
const AHCI_PORT_REG_SNTF: usize = 0x3C; // SATA Notification (SNotification)
const AHCI_PORT_REG_FBS: usize = 0x40; // FIS-based Switching Control
const AHCI_PORT_REG_DEVSLP: usize = 0x44; // Device Sleep
...
static mut CMD_TABLES: [[AhciCommandTable; NUM_CMD_SLOTS]; NUM_AHCI_PORTS] = 
    [const { [const { AhciCommandTable {
        command_fis: [0; 64],
        atapi_cmd: [0; 16],
        _reserved: [0; 48],
        prdt: [const { AhciPrdtEntry { dba: 0, _reserved: 0, dbc: 0 } }; 8]
    }}; NUM_CMD_SLOTS] }; NUM_AHCI_PORTS];
...
// Static allocations
static mut CMD_LISTS: [CommandList; NUM_AHCI_PORTS] = [const { CommandList([AhciCommandHeader {
    flags: 0, prdtl: 0, prdbc: 0, ctba: 0, _reserved: [0; 16]
}; NUM_CMD_SLOTS]) }; NUM_AHCI_PORTS];
static mut FIS_BUFFERS: [ReceivedFis; NUM_AHCI_PORTS] = [const { ReceivedFis(AhciReceivedFis {
    dsfis: [0; 28], _pad1: [0; 4], psfis: [0; 20], _pad2: [0; 12], rfis: [0; 20],
    _pad3: [0; 4], sdbfis: [0; 8], ufis: [0; 64], _pad4: [0; 96]
}) }; NUM_AHCI_PORTS];
...
impl AhciDriver {
    ...
    fn init_port(&mut self, port: usize) -> Result<(), ()> {
        let port_base = self.bar5 + 0x100 + (port * 0x80);

        // Stop port
        unsafe {
            let cmd = read_volatile((port_base + AHCI_PORT_REG_CMD) as *const u32);
            write_volatile((port_base + AHCI_PORT_REG_CMD) as *mut u32, cmd & !0x00000010); // Clear ST
            write_volatile((port_base + AHCI_PORT_REG_CMD) as *mut u32, cmd & !0x00000001); // Clear CR
            loop {
                let cmd = read_volatile((port_base + AHCI_PORT_REG_CMD) as *const u32);
                if (cmd & 0x00008000) == 0 {
                    break;
        ...
        unsafe {
            ...
            write_volatile((port_base + AHCI_PORT_REG_CLB) as *mut u32, (cmd_list_addr & 0xFFFFFFFF) as u32);
            write_volatile((port_base + AHCI_PORT_REG_CLBU) as *mut u32, (cmd_list_addr >> 32) as u32);
            write_volatile((port_base + AHCI_PORT_REG_FB) as *mut u32, (fis_addr & 0xFFFFFFFF) as u32);
            write_volatile((port_base + AHCI_PORT_REG_FBU) as *mut u32, (fis_addr >> 32) as u32);
        }

        // Enable FIS reception and start port
        unsafe {
            let cmd = read_volatile((port_base + AHCI_PORT_REG_CMD) as *const u32);
            write_volatile((port_base + AHCI_PORT_REG_CMD) as *mut u32, cmd | 0x00000010); // Set FRE
            write_volatile((port_base + AHCI_PORT_REG_CMD) as *mut u32, cmd | 0x00000001); // Set ST
        }
    ...
    fn issue_ata_cmd(&mut self, port: usize, slot: usize, lba: u64, count: u16, cmd: u8, buffer: *mut u8, write: bool) -> Result<(), ()> {
        let port_base = self.bar5 + 0x100 + (port * 0x80);
        ...
        unsafe {
            ...
            cmd_table.prdt[0].dbc = (count * 512 - 1) as u32;

            // Set up Command Header
            cmd_list.flags = 0x0005; // Command FIS length (5 dwords) | Write bit
            cmd_list.prdtl = 1;
...
impl PciDriver for AhciPciDriver {
    fn probe(&self, dev: &PciDeviceInfo) -> Result<(), ()> {
        ...
        if dev.class_code.base == 0x01 && dev.class_code.sub == 0x06 && dev.class_code.interface == 0x01 {
            // Initialize driver
