...
pub mod utils {
    ...
    pub fn keycode_to_char(keycode: u32, modifiers: Modifiers) -> Option<char> {
        ...
        // For letters, Caps Lock toggles case independently of Shift
        let effective_shift = if (0x10..=0x19).contains(&keycode) || (0x1E..=0x26).contains(&keycode) || (0x2C..=0x32).contains(&keycode) {
            shift ^ caps_lock
        } else {
            shift
        };
        ...
        match keycode as u8 {
            ...
            0x10 => Some(if effective_shift { 'Q' } else { 'q' }),
            0x11 => Some(if effective_shift { 'W' } else { 'w' }),
            0x12 => Some(if effective_shift { 'E' } else { 'e' }),
            0x13 => Some(if effective_shift { 'R' } else { 'r' }),
            0x14 => Some(if effective_shift { 'T' } else { 't' }),
            0x15 => Some(if effective_shift { 'Y' } else { 'y' }),
            0x16 => Some(if effective_shift { 'U' } else { 'u' }),
            0x17 => Some(if effective_shift { 'I' } else { 'i' }),
            0x18 => Some(if effective_shift { 'O' } else { 'o' }),
            0x19 => Some(if effective_shift { 'P' } else { 'p' }),
            0x1A => Some(if shift { '{' } else { '[' }),
            0x1B => Some(if shift { '}' } else { ']' }),
            0x1E => Some(if effective_shift { 'A' } else { 'a' }),
            0x1F => Some(if effective_shift { 'S' } else { 's' }),
            0x20 => Some(if effective_shift { 'D' } else { 'd' }),
            0x21 => Some(if effective_shift { 'F' } else { 'f' }),
            0x22 => Some(if effective_shift { 'G' } else { 'g' }),
            0x23 => Some(if effective_shift { 'H' } else { 'h' }),
            0x24 => Some(if effective_shift { 'J' } else { 'j' }),
            0x25 => Some(if effective_shift { 'K' } else { 'k' }),
            0x26 => Some(if effective_shift { 'L' } else { 'l' }),
            0x27 => Some(if shift { ':' } else { ';' }),
            0x28 => Some(if shift { '"' } else { '\'' }),
            0x29 => Some(if shift { '~' } else { '`' }),
            0x2B => Some(if shift { '|' } else { '\\' }),
            0x2C => Some(if effective_shift { 'Z' } else { 'z' }),
            0x2D => Some(if effective_shift { 'X' } else { 'x' }),
            0x2E => Some(if effective_shift { 'C' } else { 'c' }),
            0x2F => Some(if effective_shift { 'V' } else { 'v' }),
            0x30 => Some(if effective_shift { 'B' } else { 'b' }),
            0x31 => Some(if effective_shift { 'N' } else { 'n' }),
            0x32 => Some(if effective_shift { 'M' } else { 'm' }),
